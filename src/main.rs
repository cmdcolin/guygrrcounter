...
fn recognize<T: Sample + ToSample<i16>>(
    ...
    let strs = vec!["her", "girl", "go", "gerber", "huh"];
    ...
    match state {
        ...
        DecodingState::Finalized => {
            ...
            let e = recognizer.result().multiple().unwrap();
            let f = e.alternatives.iter().map(|a| a.text).collect::<Vec<&str>>();
            let g = f.join(",");
            if strs.iter().any(|a| g.contains(a)) {
                println!("yay");
            }
            println!("{}", f.join(","));
        }
